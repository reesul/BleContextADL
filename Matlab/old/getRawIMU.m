function [data, time, timeStr] = getRawIMU(filename)
%gets the raw IMU data from the file specified in the argument, returns the
%data itself, timestamp as an integer (milliseconds since midnight), and
%timestamp as its native string

    %pull all data from file
    file = fopen(filename);
    res={};
    while ~feof(file)
    ...
    for i=1:num_lines-1
        ...
        data(i,1) = str2double(line(comma(1)+1:comma(2)-1));
